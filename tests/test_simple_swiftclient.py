...
class TestSimpleSwiftClient(unittest.TestCase):
    ...
    @patch('simple_swiftclient.client.urllib2.urlopen')
    def test_should_raise_exception_if_no_storage_obj_adminurl_found(self, mock_urlopen):
        mock_urlopen.return_value.read.return_value = '{"access": {"token": {"issued_at": "2014-12-05T20:32:34.628326", "expires": "2014-12-05T21:32:34Z", "id": "1234", "tenant": {"description": "Tenant descr", "enabled": true, "id": "1234", "name": "tenant_name"}}, "serviceCatalog": [{"endpoints": [{"adminURL": "https://adminurl/v1/AUTH_12345", "region": "RegionOne", "internalURL": "https://internalurl/v1/AUTH_12345", "id": "1234", "publicURL": "https://publicurl/v1/AUTH_12345"}], "endpoints_links": [], "type": "identity", "name": "keystone"}], "user": {"username": "username", "roles_links": [], "id": "1234", "roles": [{"name": "_member_"}], "name": "User name"}, "metadata": {"is_admin": 0, "roles": ["1234"]}}}'

        cli = Client({})
        self.assertRaises(ClientException, cli.get_storage_url)
    ...
    def test_upload_should_build_request_correctly(self, mock__authenticate, mock_list_dir, mock_request):
        mock_list_dir.return_value = ['tests/fixtures/image.jpg']
